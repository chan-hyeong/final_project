...
public class StoreController {
	...
	public String store_select(HttpSession session
			, @RequestParam(name="s_code")String s_code
			, @RequestParam(name="returnURL", defaultValue="menulist.do")String returnURL
			) {
		
		StoreDAO dao=sqlsession.getMapper(StoreDAO.class);
		
		session.setAttribute("selected_store", dao.get_store_info(s_code));
		//다음페이지까지만 유효하게 해야될텐데 
